...
void mpdcontrol(const Arg *control){
    struct mpd_connection *conn;
    ...
    if(state == MPD_STATE_STOP || (state == MPD_STATE_PAUSE && control->i > 1)){
        mpd_run_play(conn);
        mpd_status_free(status);
        mpd_connection_free(conn);
    }
        ...
        if(matchcode == 0){
            if(strstr(filename, "file://") == filename && control->i > 0){ //match just at the start of the filename
        ...
        else if(matchcode == REG_NOMATCH && control->i > 0){
            mpd_run_toggle_pause(conn);
        }
        else{
